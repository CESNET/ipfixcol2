...
static int index_of_elem(const fds_iemgr_elem &elem, const fds_template &tmplt, bool rev)
{
    ...
    const fds_tfield *fields = !rev ? reinterpret_cast<const fds_tfield *>(&tmplt.fields[0]) : tmplt.fields_rev;
...
RecParser &RecParserManager::get_parser(const fds_template *tmplt)
{
    ...
    if (it == m_active_odid->end()) {
        auto [it, _] = m_active_odid->emplace(tmplt->id, RecParser(m_columns, tmplt, m_biflow_autoignore));
        return it->second;
    }
    return it->second;
}
