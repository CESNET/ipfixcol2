...
static void extract_values(ipx_msg_ipfix_t *msg, RecParser &parser, std::vector<Column>& columns,
                           Block &block, bool rev)
{
    std::size_t n_columns = columns.size();
    bool has_value;
    ValueVariant value;

    for (std::size_t i = 0; i < n_columns; i++) {
        if (columns[i].special == SpecialField::ODID) {
            has_value = true;
            ...
            if (field.data == nullptr) {
                ...
                has_value = true;
                value = get_value(columns[i].datatype, field);
            }
        }

        write_to_column(columns[i].datatype, columns[i].nullable, *block.columns[i].get(), has_value ? &value : nullptr);
    }
    block.rows++;
}
...
Plugin::process_record(ipx_msg_ipfix_t *msg, fds_drec &rec, Block &block)
{
    ...
    if (!parser.skip_fwd()) {
        extract_values(msg, parser, m_columns, block, false);
        ret++;
    }

    if (!parser.skip_rev()) {
        extract_values(msg, parser, m_columns, block, true);
        ret++;
    }
