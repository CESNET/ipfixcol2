...
/* Default number of templates stored in mapper */
#define MAPPER_MAX_TEMPLATES 20000
 ...
 * \param[out] tmgr_gc  Generated template garbage
 * \return #IPX_OK on success, if current context is not set garbage is set to NULL
 * \return #IPX_ERR_NOMEM on memory allocation error (garbage is NULL)
 */
static int
modifier_ctx_restart(ipx_modifier_t *mod, fds_tgarbage_t **tmgr_gc)
{
    ...
    rc = fds_tmgr_garbage_get(mod->curr_ctx->mgr, tmgr_gc);
    if (rc) {
...
template_store(ipx_modifier_t *mod, struct fds_template *tmplt, uint16_t original_id, ipx_msg_garbage_t **garbage, struct modified_tmplt_id *item)
{
    ...
    // First, check if template mapper is not full
    if (ipx_mapper_get_tmplt_count(mod->curr_ctx->mapper) >= MAPPER_MAX_TEMPLATES) {
        // Clear mapper and get garbage from template manager
        ipx_mapper_clear(mod->curr_ctx->mapper);
        rc = fds_tmgr_garbage_get(mod->curr_ctx->mgr, &mapper_garbage);
        if (rc != IPX_OK) {
            MODIFIER_ERROR(mod, "Failed to get garbage from template manager (%s:%d)", __FILE__, __LINE__);
            return NULL;
        }
    }

    // Try to find template in template manager
    ...
    rc = mgr_store_template(mod, tmplt, &context_garbage);
    if (rc != IPX_OK) {
