...
void ClientManager::add_connection(UniqueFd fd, unique_ptr<Decoder> decoder) {
    const char *err_str;
    ...
    if (flags == -1) {
        ...
        throw runtime_error("Failed to get flags from file descriptor: " + string(err_str));
    }

    flags |= O_NONBLOCK;
    if (fcntl(fd.get(), F_SETFL, flags) == -1) {
        ipx_strerror(errno, err_str);
        throw runtime_error("Failed to set non-blocking mode: " + string(err_str));
    }

    int borrowed_fd = fd.get();

    unique_ptr<Connection> connection(new Connection(std::move(fd), std::move(decoder)));

    auto net = &connection->get_session()->tcp.net;
    array<char, INET6_ADDRSTRLEN> src_addr_str{};
...
size_t ClientManager::wait_for_connections(Connection **connections, int max_connections) {
    ...
    unique_ptr<epoll_event []> events(new epoll_event[max_connections]);

    int ev_valid = m_epoll.wait(events.get(), max_connections, GETTER_TIMEOUT);
    if (ev_valid == -1) {
        const char *err_str;
        ipx_strerror(errno, err_str);
        throw runtime_error("Failed to wait for new data: " + string(err_str));
    }

    size_t valid_cnt = ev_valid;
...
void ClientManager::close_all_connections() {
    ...
    while (m_connections.size() != 0) {
        close_connection(m_connections.size() - 1);
    }
}

void ClientManager::close_connection(
