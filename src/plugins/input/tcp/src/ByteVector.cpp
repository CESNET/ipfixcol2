...
void ByteVector::reserve(size_t size) {
    ...
    auto ptr = m_data.get();
    unique_ptr<uint8_t [], FreeDestructor> new_ptr(
        reinterpret_cast<uint8_t *>(realloc(ptr, size))
    );

    if (!new_ptr) {
        throw runtime_error("Failed to reallocate ByteVector to size " + to_string(size));
    }
...
ByteVector ByteVector::move() noexcept {
    auto size = m_size;
    auto capacity = m_capacity;
    return ByteVector(take(), size, capacity);
}
